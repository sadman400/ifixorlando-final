...
import { formatBusinessDate, formatBusinessTime } from "@/lib/date-time";
    ...
    <div className="space-y-6 sm:space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {MONTH_NAMES[currentMonth]} {currentYear} overview
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className="stat-card rounded-xl p-4 sm:p-5"
          >
            <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground sm:text-xs">
              {stat.label}
            </p>
            <p className={`mt-1 text-2xl font-bold sm:mt-2 sm:text-3xl ${stat.color}`}>
              {stat.value}
            </p>
          </motion.div>
        ))}
      ...
      <section>
        <div className="mb-3 flex items-center justify-between sm:mb-4">
          ...
          <Link to="/appointments" className="text-xs text-primary hover:underline sm:text-sm">
            View all →
          </Link>
        </div>
        {upcoming.length === 0 ? (
          ...
          <div className="space-y-2">
            {upcoming.map((a) => (
              <Link
                ...
                to="/appointments/$id"
                params={{ id: a.id }}
                className="glass-card flex items-center justify-between rounded-lg p-3 transition-colors hover:bg-accent sm:p-4"
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-foreground">{a.customerName}</p>
                  <p className="truncate text-xs text-muted-foreground">
                    {a.iPhoneModel} · {a.description}
                  </p>
                </div>
                <div className="ml-3 text-right">
                  <p className="text-xs text-muted-foreground">
                    {formatBusinessDate(a.scheduledDate)} {formatBusinessTime(a.scheduledDate)}
                  </p>
                  <p className="text-sm font-semibold text-foreground">
                    {fmtMoney(appointmentTotal(a))}
                  </p>
                </div>
              </Link>
            ))}
