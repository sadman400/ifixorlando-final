      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {appointments.length} total repair jobs
          </p>
        </div>
        <Link to="/appointments/new">
          <Button className="glow-primary">+ New Appointment</Button>
        </Link>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {filtered.map((a) => (
            <Link
              ...
              to="/appointments/$id"
              params={{ id: a.id }}
              className="glass-card rounded-xl p-4 transition-colors hover:bg-accent"
            >
              ...
              <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
                <span className="text-xs text-muted-foreground">
                  {formatBusinessDate(a.scheduledDate)} at {formatBusinessTime(a.scheduledDate)}
                </span>
                <span className="text-sm font-semibold text-foreground">
                  ${appointmentTotal(a).toFixed(2)}
                </span>
              </div>
            </Link>
          ))}
